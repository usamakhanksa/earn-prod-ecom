...
/** Payouts & Reconciliation (Phase 6, task 6.4) — real reconciliation engine:
 * enter the actual amount a real bank/PSP statement shows, and this posts a
 * real ledger entry + computes the variance flag (±0.5% exit criterion). */
export function PayoutsReconciliationView(): React.JSX.Element {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const result = await client.get<{ items: FinancePayoutView[] }>('/payouts');
  ...
  async function reconcile(payout: FinancePayoutView): Promise<void> {
    const amountInput = window.prompt(t('finance.payouts.reconcileAmountPrompt'));
    if (amountInput === null || amountInput.trim().length === 0) return;
    const amountMinor = amountInput.trim();
    if (!/^\d+$/.test(amountMinor)) return;
    setReconcilingId(payout.id);
    try {
      await client.post(`/payouts/${payout.id}/reconcile`, { actualAmountMinor: amountMinor, receivedAt: new Date().toISOString() }, crypto.randomUUID());
      await load();
    } catch (error) {
      setLoadError(error instanceof ApiRequestError ? error.message : t('common.error'));
    } finally {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <header>
        <h1 className="text-2xl font-bold text-text-primary">{t('finance.payouts.title')}</h1>
        <p className="mt-1 text-sm text-text-secondary">{t('finance.payouts.subtitle')}</p>
      </header>
      ...
      <div className="overflow-x-auto rounded-lg border border-border-subtle">
        {payouts === null ? (
          ...
          <p className="p-4 text-sm text-text-secondary">{t('finance.payouts.empty')}</p>
        ) : (
          <table className="w-full text-start text-sm">
            <thead className="bg-surface-2 text-xs uppercase text-text-secondary">
              <tr>
                ...
                <th scope="col" className="px-4 py-2 text-start"><span className="sr-only">{t('finance.payouts.reconcileButton')}</span></th>
              </tr>
            </thead>
            <tbody>
              {payouts.map((payout) => (
                <tr key={payout.id} className="border-t border-border-subtle">
                  ...
                  <td className="px-4 py-2">
                    <Button variant="secondary" size="sm" loading={reconcilingId === payout.id} onClick={() => void reconcile(payout)}>
                      {t('finance.payouts.reconcileButton')}
                    </Button>
                  </td>
                </tr>
              ))}
